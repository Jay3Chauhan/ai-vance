...
class _DownloadedModelsScreenState extends ConsumerState<DownloadedModelsScreen>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      final ok = await requestStoragePermissionForDownloads();
      if (!mounted) return;
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed && Platform.isAndroid) {
      hasStoragePermissionForDownloads().then((ok) {
        if (mounted) {
          setState(() => _hasPermission = ok);
          ref.invalidate(downloadedModelsProvider);
        }
                ...
                return Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        'Models you download into AIVance will appear here.',
                      ...
                      if (Platform.isAndroid && !_hasPermission) ...[
                        ...
                        FilledButton.icon(
                          ...
                          onPressed: () async {
                            final ok = await openManageStorageSettings();
                            if (mounted) {
                              setState(() => _hasPermission = ok);
                              ref.invalidate(downloadedModelsProvider);
                            }
                              ...
                              onPressed: () async {
                                final remove = await showDialog<bool>(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                    ...
                                    content: Text(
                                      'Delete "${m.name}" from storage? This will remove the file from AIVance.',
                                      style: GoogleFonts.plusJakartaSans(),
                                    ),
                                    actions: [
                                      TextButton(
